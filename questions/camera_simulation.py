...
def calculate_camera_matrix(tx, ty, tz, alpha, beta, gamma, fx, fy, skew, u, v):
    ...
    intrinsic_matrix = np.array([[fx, skew, u],
                  [0, fy, v],
                  [0, 0, 1]])
    ...
    y_rotation = np.array([[np.cos(beta), 0, np.sin(beta)],
                           [0, 1, 0],
                           [-np.sin(beta), 0, np.cos(beta)]])
    z_rotation = np.array([[np.cos(gamma), -np.sin(gamma), 0],
    ...
    return camera_matrix, intrinsic_matrix, extrinsic_matrix
